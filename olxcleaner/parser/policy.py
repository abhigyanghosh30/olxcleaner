        ...
        if url_name is None:
            if edxobj.needs_url_name and not edxobj.broken:
                ...
                errorstore.add_error(
                    MissingURLName(edxobj.filenames[0], edxobj=edxobj)
                )
        else:
            # Record the name
...
def merge_policy(policy, url_names, errorstore):
    ...
    for entry in policy:
        ...
        if edxobj is None:
            errorstore.add_error(
                PolicyRefNotFound(
                    "policy.json", objtype=objtype, url_name=url_name
                )
            )
            continue
        ...
        if not isinstance(policy[entry], dict):
            errorstore.add_error(
                BadEntry("policy.json", objtype=objtype, url_name=url_name)
            )
            continue
...
def validate_grading_policy(grading_policy, errorstore):
    ...
    if "GRADER" not in grading_policy:
        errorstore.add_error(
            GradingPolicyIssue(
                "grading_policy.json",
                msg="'GRADER' entry not found in grading policy",
            )
        )
        ...
        errorstore.add_error(
            GradingPolicyIssue(
                "grading_policy.json",
                msg="'GRADER' entry not in grading policy is not a list",
            )
        )
    else:
        weightsum = 0
        ...
        for idx, entry in enumerate(grading_policy["GRADER"]):
            if "drop_count" not in entry:
                ...
                errorstore.add_error(
                    GradingPolicyIssue("grading_policy.json", msg=msg)
                )
            ...
            if "short_label" in entry and not isinstance(
                entry["short_label"], str
            ):
            ...
            if "weight" not in entry:
                ...
                isinstance(entry["weight"], int)
                or isinstance(entry["weight"], float)
            ):
    ...
    if "GRADE_CUTOFFS" not in grading_policy:
        errorstore.add_error(
            GradingPolicyIssue(
                "grading_policy.json",
                msg="'GRADE_CUTOFFS' entry not found in grading policy",
            )
        )
        ...
        errorstore.add_error(
            GradingPolicyIssue(
                "grading_policy.json",
                msg="'GRADE_CUTOFFS' entry not in grading policy is not a dictionary",
            )
        )
    else:
        ...
        for entry in grading_policy["GRADE_CUTOFFS"]:
            if entry in ["Pass", "A", "B", "C", "D"]:
                ...
                if not (
                    isinstance(grading_policy["GRADE_CUTOFFS"][entry], int)
                    or isinstance(
                        grading_policy["GRADE_CUTOFFS"][entry], float
                    )
                ):
                    ...
                    grading_policy["GRADE_CUTOFFS"][entry] > 1
                    or grading_policy["GRADE_CUTOFFS"][entry] < 0
                ):
                    # We sometimes use 'Pass' < 0 to hide the pass line on the plot
                    if not (
                        entry == "Pass"
                        and grading_policy["GRADE_CUTOFFS"][entry] < 0
                    ):
        ...
        if clean:
            if len(grading_policy["GRADE_CUTOFFS"]) == 0:
                ...
                "Pass" in grading_policy["GRADE_CUTOFFS"]
                and len(grading_policy["GRADE_CUTOFFS"]) > 1
            ):
